Linkedin = {
    config: {
      ...
      //here {{name}} does not capture value of name, instead shows "Hey undefined, I'm looking forward ..."
    },
    ...
    clickAddNote: function (data, config) {
      ...
      var addNoteButton = Array.prototype.filter.call(buttons, function (el) {
        return el.textContent.trim() === "Send without a note"; //create list of buttons with this values
      });
    ...
    nextPage: function (config) {
      var pagerButton = document.getElementsByClassName(
        ...
        "artdeco-button artdeco-button--muted artdeco-button--icon-right artdeco-button--1 artdeco-button--tertiary ember-view artdeco-pagination__button artdeco-pagination__button--next",
      );
      if (
        !pagerButton ||
